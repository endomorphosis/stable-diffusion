...
for i in $(seq 1 $NUM_GPUS); do
    ...
    if [ $(($GPU_MEMORY_FREE)) -lt $(($MIN_MEMORY)) ]; then
        ...
        #python -u webui.py $WEBUI_ARGS --realesrgan-dir=/home/diffusion/hlky/stable-diffusion/src/realesrgan/ --share --gpu "$CUR_GPU" --port "$CUR_PORT" &
    fi
done
#remove last comma
